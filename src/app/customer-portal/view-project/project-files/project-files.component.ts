...
export class ProjectFilesComponent implements OnInit {
      ...
      click: (tree, node, event) => {
        ...
        if (node !== this.activeFolderNode) {
          ...
          if (node.id !== this.currentProject["project_id"]) {
            if (
              !this.prevClickEventDate ||
              new Date().getTime() - this.prevClickEventDate.getTime() >= 400
            ) {
              this.loadFiles();
            }
          }
  ...
  fileGridColumns = [
    ...
    },
    {
      ...
      dataField: "folder_original_filename",
      minWidth: 200,
      width: 300,
      allowEditing: false,
    },
  ...
  get isFavoriteDocument(): boolean {
    ...
    if (selectedRowKeys.length === 1) {
      if (
        this.favorites.find(
          (favorite) => favorite["favorite_id"] === selectedRowKeys[0]["doc_id"]
        )
      ) {
        return true;
      }
  ...
  ngOnInit() {
    ...
    if (this.dataStore.currentUser) {
      ...
      if (this.activatedRoute.snapshot.queryParams["viewDocumentId"] != null) {
        this.onViewDocumentDetails(
          this.activatedRoute.snapshot.queryParams["viewDocumentId"]
        );
      } else if (
        this.activatedRoute.snapshot.queryParams["editDocumentId"] != null
      ) {
        this.onEditDocument(
          this.activatedRoute.snapshot.queryParams["editDocumentId"]
        );
      }
    } else {
      this.dataStore.authenticationState.subscribe((value) => {
        if (value) {
          ...
          if (
            this.activatedRoute.snapshot.queryParams["viewDocumentId"] != null
          ) {
            this.onViewDocumentDetails(
              this.activatedRoute.snapshot.queryParams["viewDocumentId"]
            );
  ...
  initialize() {
    ...
    if (this.dataStore.currentUser["user_id"] != null) {
      this.apiService
        ...
        .catch((err) => {
          this.notificationService.error("Error bb", err, {
            timeOut: 3000,
  ...
  loadRootFolders() {
    this.apiService
      .getFolderChildren(this.currentProject["project_id"], "", "")
      .then((res: any[]) => {
        this.folderNodes = [
          {
        ...
        setTimeout(() => {
          this.folderTree.treeModel
            .getNodeById(this.currentProject["project_id"])
            .expand();

          const currentPlansFolder = this.folderNodes[0].children.find(
            (child) => child.name === "Plans-Current"
          );
          if (currentPlansFolder) {
            this.activeFolderNode = this.folderTree.treeModel.getNodeById(
              currentPlansFolder.id
            );
            this.activeFolderNode.toggleActivated();
            this.loadSubFolders(true);
          } else {
            const currentPlansFolder = this.folderNodes[0].children.find(
              (child) => child.name === "Plans-All"
            );
            this.activeFolderNode = this.folderTree.treeModel.getNodeById(
              currentPlansFolder.id
            );
  ...
  loadSubFolders(loadFiles: boolean = false) {
    ...
    if (this.activeFolderNode != null) {
      this.apiService
        .getFolderChildren("", "", this.activeFolderNode.id, timezone)
        .then((res: any[]) => {
          this.activeFolderNode.data.children = res
            .filter((child) => child["child_type"] === "folder")
            .map((folder) => {
              return {
                id: folder["folder_id"],
                name: folder["folder_name"],
          ...
          setTimeout(() => {
            this.folderTree.treeModel
              .getNodeById(this.activeFolderNode.id)
              .expand();
          });
        })
        .catch((err) => {
  ...
  onViewDocument() {
    ...
    if (selectedRowKeys.length === 0) {
      ...
      this.notificationService.error(
        "Multiple Selection",
        "Please select a single document.",
        { timeOut: 3000, showProgressBar: false }
      );
      return;
    }
    ...
    const isComparison = this.activeFolderNode.data["name"].includes(
      "Comparison"
    );
    const {
    ...
    window.open(
      `${window["env"].docViewerBaseUrl}?project_id=${
        this.currentProject["project_id"]
      }&user_id=${userId}&folder_id=${selectedRowKeys[0]["folder_id"]}&doc_id=${
        selectedRowKeys[0]["doc_id"]
      }&doc_type=${isComparison ? "comparison" : "normal"}`,
      "_blank"
    );
  }
  ...
  onViewDocumentDetails(docId?: string) {
    if (docId == null) {
      ...
      this.documentDetailModal.initialize(
        this.currentProject,
        submission,
        selectedRowKeys[0],
        false
      ...
      this.apiService
        ...
        .catch((error) => {
          this.notificationService.error(
            "Error",
            "Failed to show the document details modal!",
            { timeOut: 3000, showProgressBar: false }
          );
          return;
        });
  ...
  onDownloadDocument() {
    ...
    this.projectApiService
      .getDocument(selectedRowKeys[0]["doc_id"])
      .then((doc: any) => {
        const ext = doc["file_key"].substr(
          doc["file_key"].lastIndexOf(".") + 1
        );
        return this.amazonService.getPresignedUrlWithOriginalFileName(
          doc["bucket_name"],
          doc["file_key"],
          `${doc["doc_name"]}.${ext}`
        );
  ...
  onDownloadFolder() {
    if (!this.activeFolderNode) {
      return this.notificationService.error("No Selection", "Please a folder", {
        timeOut: 3000,
        showProgressBar: false,
      });
    }
  ...
  onDownloadProject() {
    this.projectApiService
      .getPublishedLink(this.currentProject["project_id"])
      .then((url: string) => {
        const downloadUrl = url.replace("dl=0", "dl=1");
        window.open(downloadUrl, "_blank");
      })
  ...
  onToggleFavorite() {
    ...
    if (this.isFavoriteDocument) {
      const favoriteIndex = this.favorites.findIndex(
        (favorite) => favorite["favorite_id"] === docId
      );
      const favorite = this.favorites[favoriteIndex];

      this.apiService
        .removeUserFavorite(favorite["user_favorite_id"])
        .then((_) => {
      ...
      this.apiService
        ...
        .then((user_favorite_id: string) => {
          ...
          this.notificationService.success("Success", "Added to favorites", {
            timeOut: 3000,
            showProgressBar: false,
          });
        })
  ...
  onViewTransactionLogs() {
    ...
    this.transactionLogsModal.initialize(
      this.currentProject,
      { submission_id: selectedRowKeys[0]["submission_id"] },
      selectedRowKeys[0]
    );
  }
