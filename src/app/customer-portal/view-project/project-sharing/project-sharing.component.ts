...
export class ProjectSharingComponent implements OnInit {
  ...
  get projectId() {
    return this.activatedRoute.parent.snapshot.params["project_id"];
  }
  get isPubliclyShared() {
    return this.originData.find(
      (sharedProject) =>
        sharedProject["public"] &&
        sharedProject["project_id"] === this.projectId
    );
  }
  ...
  load() {
    this.projectSharingApi
      .findShareUsers(this.projectId, this.dataStore.currentUser.user_id)
      .then((users: any[]) => {
  ...
  removeShare(shared_project_id: string) {
    ...
    this.projectSharingApi
      ...
      .then((res) => {
        this.spinner.hide();
        this.notificationService.success("Success", "Sharing Removed", {
          timeOut: 3000,
          showProgressBar: false,
        });
  ...
  getDownloadLink = () => {
    return new Promise<string>((resolve, reject) => {
      this.projectsApi
        .getPublishedLink(this.dataStore.currentProject["project_id"])
        .then((url: string) => {
  ...
  onRemoveShare() {
    ...
    if (selectedShares.length !== 1) {
      this.notificationService.error("Error", "Please select one record!", {
        timeOut: 3000,
        showProgressBar: false,
      });
    } else {
      this.removeShareModal.initialize(
        this,
        selectedShares[0]["shared_project_id"]
      );
    }
  }
  ...
  copyToClipboard(value: string) {
    ...
    this.notificationService.success("Success", "Copied to clipboard", {
      timeOut: 3000,
      showProgressBar: false,
    });
  }
