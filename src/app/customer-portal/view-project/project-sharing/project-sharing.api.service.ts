...
export class ProjectSharingApi {
  public findSharedProjects(params, timezone: string) {
    return new Promise((resolve, reject) => {
      axios.get(window['env'].apiBaseUrl + `/FindSharedProjects?${queryString.stringify(params)}`)
        .then(res => {
          res.data = res.data.map(project => {
            project['project_city_state'] = `${project['project_state']}, ${project['project_city']}`;
            project['edit_datetime'] = this.convertToTimeZoneString(project['edit_datetime'], timezone);
            project['create_datetime'] = this.convertToTimeZoneString(project['create_datetime'], timezone);
            project['project_bid_datetime'] = this.convertToTimeZoneString(project['project_bid_datetime'], timezone);
  ...
  public findShareUsers(project_id: string) {
    return new Promise((resolve, reject) => {
      axios.get(window['env'].apiBaseUrl + `/FindSharedProjects?project_id=${project_id}&status=active`)
        .then(res => {
          resolve(res.data);
        })
        .catch(err => {
  ...
  public updateSharedProject(params: any) {
    return new Promise((resolve, reject) => {
      axios.post(window['env'].apiBaseUrl + '/UpdateSharedProject', queryString.stringify(params))
        .then(res => {
          resolve();
        })
        .catch(err => {
