...
export class CompanyEmployeesComponent implements OnInit {
    ...
    this.toolbarConfig = {
      ...
      others: {
        ...
        editContact: {
          type: "normal",
          text: "Edit Contact",
          onClick: () => this.toolbarEditContactAction(),
        },
        viewTransactionLog: {
          type: "normal",
  ...
  gridProjectLoadAction(loadOptions) {
    return new Promise((resolve, reject) => {
      ...
      const findProjects = this.myCalenderApi.findCompanyContact(
        this.customer_id,
      ...
      Promise.all([findProjects])
        .then(([projects, dataViewFieldSettings]) => {
          ...
          if (!this.projectViewTypeSelected) {
            this.projectGridColumns = [
              ...
              {
                dataField: "contact_display_name",
                caption: "Name",
                width: 200,
                minWidth: 100,
                allowEditing: false,
              ...
              {
                dataField: "contact_mobile_phone",
                caption: "Mobile",
                width: 200,
                minWidth: 100,
                allowEditing: true,
        ...
        .catch((error) => {
          ...
          this.notificationService.error("Error fgd", error, {
            timeOut: 3000,
  ...
  toolbarEditContactAction() {
    ...
    if (selectedRowKeys.length === 0) {
      ...
      this.notificationService.error(
        "Multiple Selection",
        "Please select just one Contact!",
        { timeOut: 3000, showProgressBar: false }
      );
      return;
    }
    const selectedRows = this.projectGridContent.filter(
      ({ contact_id: contactId }) => selectedRowKeys.includes(contactId)
    );
    console.log(selectedRows);
    this.addContactModal.initialize(this, selectedRows[0]);
  }
  ...
  toolbarViewContactAction() {
    ...
    window.open(
      `/customer-portal/view-employee/${selectedRows[0].contact_id}/overview`,
      "_blank"
    );
  }
  ...
  toolbarViewTransactionLogAction() {
    ...
    const selectedRows = this.projectGridContent.filter(
      ({ contact_id: projectId }) => selectedRowKeys.includes(projectId)
    );
