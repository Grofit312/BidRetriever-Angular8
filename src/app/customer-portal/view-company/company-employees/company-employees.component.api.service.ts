...
export class ContactApi {
  public getEmployee(contact_id: string, timezone: string) {
    return new Promise((resolve, reject) => {
      axios
        .get(
          `${window["env"].apiBaseUrl}/GetContact?contact_id=${contact_id}`,
          {
  ...
  public createContact(params: any) {
    return new Promise((resolve, reject) => {
      axios
        .post(
          window["env"].apiBaseUrl + "/CreateContact",
  ...
  public updateContact(params: any) {
    return new Promise((resolve, reject) => {
      axios
        .post(
          window["env"].apiBaseUrl + "/UpdateContact",
          queryString.stringify(params),
          {
            validateStatus: (status) => {
              return status === 200 || status === 400;
