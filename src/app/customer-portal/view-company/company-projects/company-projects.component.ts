...
export class CompanyProjectsComponent implements OnInit, AfterViewInit {
  ...
  gridProjectLoadAction(loadOptions) {
    return new Promise((resolve, reject) => {
      ...
      const findProjects = this.myCalenderApi.findProjectList(
        this.source_company_id,
        this.selectedUserId === "all-users" ? null : this.selectedUserId,
        ...
        .then(([projects, dataViewFieldSettings]) => {
          ...
          if (!this.projectViewTypeSelected) {
            this.projectGridColumns = [
              ...
              {
                dataField: "project_admin_user_fullname",
                caption: "Admin User",
                minWidth: 150,
            ...
            if (
              dataViewFieldSettings != undefined &&
              dataViewFieldSettings != null
            ) {
              (dataViewFieldSettings as any[]).forEach((viewFieldSetting) => {
                ...
                if (
                  viewFieldSetting.data_view_field_name.includes("datetime")
                ) {
            ...
            console.log("projectGridColumns", this.projectGridColumns);
          }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if ("project_name" in values) {
          ...
          const matchedUser = this.projectGridEditorTemplateSource.adminUserEmail.find(
            ({ user_email }) =>
              user_email === values["project_admin_user_fullname"]
          );
          this.apiService
            .updateProject(key, {
            ...
            .then((res) => {
              ...
              this.projectGridContent[updateIndex]["project_admin_user_email"] =
                values["project_admin_user_fullname"];
              return resolve();
            })
            .catch((error) => {
          ...
          this.apiService
            ...
            .then((res) => {
              this.notificationService.success(
                "Success",
                "Project Admin User has been updated",
                { timeOut: 3000, showProgressBar: false }
              );
              this.projectGridContent[updateIndex][
                "project_admin_user_fullname"
  ...
  onRowDoubleClicked(event: any) {
    window.open(
      `/customer-portal/view-project/${event["data"]["project_id"]}`,
      "_blank"
    );
  }
