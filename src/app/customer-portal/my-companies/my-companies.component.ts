...
export class MyCompaniesComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild('companyToolbar', { static: false }) companyToolbar: DxToolbarComponent;

  @ViewChild('companyToolbarViewType', { static: false }) companyToolbarViewType: DxSelectBoxComponent;

  private PROJECT_TOOLBAR_INITIAL_VIEW = 'BidRetriever_Company_Page_Toolbar_Initial_View';
      ...
      companyViewType: {
        ...
        dataSource: new DataSource({
          store: new CustomStore({
            key: 'view_id',
            loadMode: 'raw',
            load: (loadOptions) => this.toolbarCompanyViewTypeLoadAction(loadOptions)
          })
        }),
        ...
        onValueChanged: (event) => {
          ...
          if (this.companyViewTypeSelected !== event.value) {
            ...
            localStorage.setItem(this.PROJECT_TOOLBAR_INITIAL_VIEW, this.companyViewTypeSelected == null ? '' : this.companyViewTypeSelected);
  ...
  ngAfterViewInit() {
    ...
    if (this.companyGrid && this.companyGrid.instance) {
      this.companyGrid.instance.columnOption('command:select', 'allowFixing', true);
    }
  }
  ...
  findMatchedCompanyAdminUserDisplayName(email: string) {
    if (!this.companyGridEditorTemplateSource || !this.companyGridEditorTemplateSource.adminUserEmail) {
      return '';
    }

    const matchedUser = this.companyGridEditorTemplateSource.adminUserEmail.find((user) => user.user_email === email);
    if (matchedUser) {
  ...
  load() {
    ...
    if (initialDataViewSelected) {
      ...
      if (this.companyToolbarViewType && this.companyToolbarViewType.instance && this.companyViewTypeSelected) {
        this.companyToolbarViewType.instance.getDataSource().reload().then((data) => {
  ...
  loadCurrentOffice() {
    this.userInfoApiService.findUsers(this.dataStore.currentUser['customer_id'])
      .then((users: any[]) => {
        ...
        this.companyGridEditorTemplateSource.adminUserEmail = emails.sort((firstUser, secondUser) => {
  ...
  private getGridCompanyContentByLoadOption(loadOptions) {
    ...
    if (loadOptions.sort && loadOptions.sort.length > 0) {
      companies = companies.sort((first, second) => {
        const sortColumnOption = this.companyGridColumns.find((column) => column.dataField === loadOptions.sort[0].selector);
    ...
    if (this.searchText) {
      companies = companies.filter((company) => {
        const isMatched = Object.keys(company).map(key => company[key]).some(item => item.toString().toLowerCase().includes(this.searchText));
        return isMatched;
      });
    }
    ...
    gridCompanyLoadAction(loadOptions) {
        return new Promise((resolve, reject) => {
            if (this.companyGridContentLoaded) {
                const filteredCompanies = this.getGridCompanyContentByLoadOption(loadOptions);
                return resolve({
            ...
            const findCompanies = this.selectedUserId === 'all-users'
            ? this.apiService.findCompaniesByCustomerId(this.dataStore.currentUser['customer_id'], this.dataStore.currentCustomer['customer_timezone'] || 'eastern', this.companyViewTypeSelected)
            : this.apiService.findCompaniesByUserId(this.selectedUserId, this.selectedCustomerId, this.dataStore.currentCustomer['customer_timezone'] || 'eastern', this.companyViewTypeSelected);

            const findDataViewFieldSettings = this.apiService.findDataViewFieldSettings(this.companyViewTypeSelected);
            ...
            .then(([companies, dataViewFieldSettings]) => {
                ...
                if (!this.companyViewTypeSelected) {
                    ...
                    { dataField: 'company_id', dataType: 'number', caption: 'Company Id', width: 250, visible: false, allowEditing: false },
                    { dataField: 'company_name', caption: 'Company Name', width: 400, minWidth: 250, allowEditing: true },
                    { dataField: 'company_admin_displayname', caption: 'Company Admin', minWidth: 150, allowEditing: false, cellTemplate: 'companyAdminUserEmailCell', editCellTemplate: 'companyAdminUserEmailEditor' },
                    { dataField: 'company_record_source', caption: 'Record Source', minWidth: 150, allowEditing: false },
                    { dataField: 'company_email', caption: 'Company email', minWidth: 150, allowEditing: false },
                    { dataField: 'company_employee_number', caption: 'Company Employee Number', minWidth: 150, allowEditing: false },
                    { dataField: 'company_domain', caption: 'Company Domain', minWidth: 150, allowEditing: false },
                    { dataField: 'company_phone', caption: 'Company Phone', minWidth: 150, allowEditing: false },
                    { dataField: 'company_revenue', caption: 'Company Revenue', minWidth: 150, allowEditing: false },
                    { dataField: 'company_timezone', caption: 'Company Timezone', minWidth: 150, allowEditing: false },
                    { dataField: 'company_type', caption: 'Company Type', minWidth: 150, allowEditing: false, editCellTemplate: 'typeEditor' },
                    { dataField: 'company_website', caption: 'Company Website', minWidth: 150, allowEditing: false },

                   { dataField: 'company_city_state', caption: 'City/State', width: 150, minWidth: 100, allowEditing: false },
                   { dataField: 'company_zip', caption: 'Company Zipcode', minWidth: 150, allowEditing: false },
                   ...
                   // { dataField: 'auto_update_status', caption: 'Automatic Updates', width: 180, minWidth: 150, allowEditing: true, editCellTemplate: 'autoUpdateStatusEditor' },
                    ...
                    { dataField: 'status', caption: 'Status', width: 100, minWidth: 100, allowEditing: false, editCellTemplate: 'statusEditor' },
                   ...
                   // { dataField: 'company_process_status', caption: 'Processing Status', minWidth: 100, allowEditing: false },
                    ...
                    const editingAllowedColumns = [
                      'company_name', 'company_admin_user_email', 'company_bid_datetime', 'auto_update_status', 'company_notes',
                      'company_stage', 'company_timezone', 'company_contract_type', 'company_segment', 'company_building_type', 'company_labor_requirement',
                      'company_value', 'company_size', 'company_construction_type', 'company_award_status', 'company_assigned_office_name',
                      'company_number'
                    ];
                    (dataViewFieldSettings as any[]).forEach((viewFieldSetting) => {
                        ...
                        switch (viewFieldSetting.data_view_field_name) {
                            case 'company_admin_user_email':
                              ...
                              newGridColumn['editCellTemplate'] = 'companyAdminUserEmailEditor';
                              break;
                            case 'company_assigned_office_name': newGridColumn['editCellTemplate'] = 'companyAssignedOfficeNameEditor'; break;
                            case 'auto_update_status': newGridColumn['editCellTemplate'] = 'autoUpdateStatusEditor'; break;
                            case 'company_stage': newGridColumn['editCellTemplate'] = 'companyStageEditor'; break;
                            case 'company_timezone': newGridColumn['editCellTemplate'] = 'companyTimezoneEditor'; break;
                            case 'company_contract_type': newGridColumn['editCellTemplate'] = 'companyContractTypeEditor'; break;
                            case 'company_segment': newGridColumn['editCellTemplate'] = 'companySegmentEditor'; break;
                            case 'company_building_type': newGridColumn['editCellTemplate'] = 'companyBuildingTypeEditor'; break;
                            case 'company_labor_requirement': newGridColumn['editCellTemplate'] = 'companyLaborRequirementEditor'; break;
                            case 'company_construction_type': newGridColumn['editCellTemplate'] = 'companyConstructionTypeEditor'; break;
                            case 'company_award_status': newGridColumn['editCellTemplate'] = 'companyAwardStatusEditor'; break;
                        }
  ...
  gridCompanyUpdateAction(key, values) {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const updateIndex = this.companyGridContent.findIndex((company) => company.company_id === key);
            // return reject('Failed to update.');
            if ('company_name' in values) {
                const validCompanyName = this.validationService.validateCompanyName(values['company_name']);
                if (validCompanyName.length === 0) {
                    return reject('Company name cannot be empty.');
                } else {
                    this.apiService.updateCompany(key, {
                    company_name : validCompanyName,
                    user_id : this.dataStore.currentUser['user_id'],
                    customer_id: this.dataStore.currentUser['customer_id'],
                    }).then((res) => {
                        this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
                        this.companyGridContent[updateIndex]['company_name'] = values['company_name'];
                        return resolve();
                    }).catch((error) => {
                        return reject('Failed to update.');
                    });
                ...
                this.apiService.updateCompany(key, {
                    ...
                    this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
                    this.companyGridContent[updateIndex]['company_record_source'] = values['company_record_source'];
                    return resolve();
                }).catch((error) => {
                    return reject('Failed to update.');
                });
            }
            else if ('company_domain' in values){
                this.apiService.updateCompany(key, {
                    company_name : values['company_domain'],
                    user_id : this.dataStore.currentUser['user_id'],
                    customer_id: this.dataStore.currentUser['customer_id'],
                }).then((res) => {
                    this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
                    this.companyGridContent[updateIndex]['company_domain'] = values['company_domain'];
                    return resolve();
                }).catch((error) => {
                    return reject('Failed to update.');
                });
            }
            else if ('company_email' in values){
                this.apiService.updateCompany(key, {
                    ...
                    this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
                    this.companyGridContent[updateIndex]['company_email'] = values['company_email'];
                    return resolve();
                }).catch((error) => {
                    return reject('Failed to update.');
                });
            }
            else if ('company_employee_number' in values){
                this.apiService.updateCompany(key, {
                    company_name : values['company_employee_number'],
                    user_id : this.dataStore.currentUser['user_id'],
                    customer_id: this.dataStore.currentUser['customer_id'],
                }).then((res) => {
                    this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
                    this.companyGridContent[updateIndex]['company_employee_number'] = values['company_employee_number'];
                    return resolve();
                }).catch((error) => {
                    return reject('Failed to update.');
                });
            } else if ('company_phone' in values){
                this.apiService.updateCompany(key, {
                    company_name : values['company_phone'],
                    user_id : this.dataStore.currentUser['user_id'],
                    customer_id: this.dataStore.currentUser['customer_id'],
                }).then((res) => {
                    this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
                    this.companyGridContent[updateIndex]['company_phone'] = values['company_phone'];
                    return resolve();
                }).catch((error) => {
                    return reject('Failed to update.');
                });
            }
             else if ('company_revenue' in values){
                this.apiService.updateCompany(key, {
                    company_name : values['company_revenue'],
                    user_id : this.dataStore.currentUser['user_id'],
                    customer_id: this.dataStore.currentUser['customer_id'],
                }).then((res) => {
                    this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
                    this.companyGridContent[updateIndex]['company_revenue'] = values['company_revenue'];
                    return resolve();
                }).catch((error) => {
                    return reject('Failed to update.');
                });
            }
            else if ('company_type' in values){
                this.apiService.updateCompany(key, {
                    company_name : values['company_type'],
                    user_id : this.dataStore.currentUser['user_id'],
                    customer_id: this.dataStore.currentUser['customer_id'],
                }).then((res) => {
                    this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
                    this.companyGridContent[updateIndex]['company_type'] = values['company_type'];
                    return resolve();
                }).catch((error) => {
                    return reject('Failed to update.');
                });
            }
            else if ('company_website' in values){
                this.apiService.updateCompany(key, {
                    company_name : values['company_website'],
                    user_id : this.dataStore.currentUser['user_id'],
                    customer_id: this.dataStore.currentUser['customer_id'],
                }).then((res) => {
                    this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
                    this.companyGridContent[updateIndex]['company_website'] = values['company_website'];
                    return resolve();
                }).catch((error) => {
                    return reject('Failed to update.');
                });
                ...
                this.apiService.updateCompany(key, {
                    ...
                    this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
                    this.companyGridContent[updateIndex]['company_zip'] = values['company_zip'];
                    return resolve();
                }).catch((error) => {
                    return reject('Failed to update.');
                });
  ...
  toolbarCompanyViewTypeLoadAction(loadOptions) {
    return new Promise((resolve, reject) => {
      ...
      this.apiService.findDataViews('companies', customerId)
        .then((viewTypes: any[]) => {
          viewTypes.push({
              ...
              res = res.map((item) => {
                ...
                if (!item.user_displayname) {
                  if (!item.user_firstname && !item.user_lastname) {
                    ...
                    item.user_displayname = `${item.user_lastname ? item.user_lastname + ', ' : ''}${item.user_firstname}`;
                  }
                }
                return item;
              });
  ...
  toolbarViewCompanyAction() {
    ...
    if (selectedRowKeys.length === 0)
     {
      this.notificationService.error('No Selection', 'Please select one company!', { timeOut: 3000, showProgressBar: false });
      return;
    } else if (selectedRowKeys.length == 1)   
    {
      const selectedRows = this.companyGridContent.filter(({ company_id: companyId }) => selectedRowKeys.includes(companyId));
     ...
     {  
      Swal.fire({
        title: 'You have selected ' + selectedRowKeys.length + ' companies and selected the view option',
        ...
        for (let i = 0; i <= selectedRowKeys.length; i++) {
          const selectedRows = this.companyGridContent.filter(({ company_id: companyId }) => selectedRowKeys[i].includes(companyId));
          // window.open(`/customer-portal/view-company/companydetails/${selectedRows[0].company_id}`, '_blank');
           window.open(`/customer-portal/view-company/${selectedRows[0].company_id}/overview`, '_blank');
        }}
          ...
          Swal.fire(
            'Cancelled',
            'Your Data default display)'
          )
        }               
     });
  }
  else if(selectedRowKeys.length >= 20){
     this.notificationService.error('Multiple Selection', 'you have selected more than 20 companies. The system can only display 20 at a time. Please select less companies and try again.', { timeOut: 3000, showProgressBar: false });
     return;    
  }
  }
  ...
  toolbarViewCompanyDocumentsAction() {
    ...
    if (selectedRowKeys.length === 0) {
      ...
      this.notificationService.error('Multiple Selection', 'Please select just one company!', { timeOut: 3000, showProgressBar: false });
      return;
    }

    const selectedRows = this.companyGridContent.filter(({ company_id: companyId }) => selectedRowKeys.includes(companyId));
    const { currentUser: { user_id: userId } } = this.dataStore;
    window.open(`${window['env'].docViewerBaseUrl}?company_id=${selectedRows[0].company_id}&user_id=${userId}`, '_blank');
  }
  ...
  toolbarDeleteCompanyAction() {
    const { selectedRowKeys } = this.companyGrid;
    if (selectedRowKeys.length === 0) {
      this.notificationService.error('No Selection', 'Please select at least one company!', { timeOut: 3000, showProgressBar: false });
      return;
    }

    const selectedRows = this.companyGridContent.filter(({ company_id: companyId }) => selectedRowKeys.includes(companyId));
    const selectedCompanies = selectedRows.filter(company => company.status !== 'deleted' && company.status !== 'archived');
  ...
  toolbarAddDocumentsToCompanyAction() {
    ...
    if (selectedRows[0].status === 'deleted' || selectedRows[0].status === 'archived') {
      this.notificationService.error('Error', 'You cannot add submission to deleted/archived company!', { timeOut: 3000, showProgressBar: false });
      return;
    }
  ...
  toolbarViewPublishedCompanyAction() {
    ...
    const selectedRows = this.companyGridContent.filter(({ company_id: companyId }) => selectedRowKeys.includes(companyId));
    this.apiService.getPublishedLink(selectedRows[0].company_id)
      .then((url: string) => {
  ...
  onCellValueChanged(event: any) {
    ...
    // update company
    this.apiService.updateCompany(companyId, { [columnName]: newValue })
      .then(res => {
        this.notificationService.success('Success', 'Company has been updated', { timeOut: 3000, showProgressBar: false });
      })
      .catch(err => {
  ...
  onRowDoubleClicked(event: any) {
    window.open(`/customer-portal/view-company/${event['data']['company_id']}`, '_blank');
  }
