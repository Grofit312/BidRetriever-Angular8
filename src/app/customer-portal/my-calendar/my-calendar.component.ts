...
export class MyCalendarComponent implements OnInit {
  @ViewChild("targetScheduler", { static: false })
  ...
  toolbarConfig: any = {};
  calendarViewModes: any[] = [
    { value: "my-active", label: "My Events" },
    { value: "my-inactive", label: "My Inactive Events" },
    { value: "all-active", label: "All Events" },
    { value: "all-inactive", label: "All Inactive Events" },
  ];
  ...
  constructor(
    ...
    this.toolbarConfig = {
      eventViewType: {
        ...
        onValueChanged: (e) => {
          this.calendarViewMode = e.value;
          this.onChangeCalendarViewMode();
        },
      },
      ...
      others: {
        ...
        viewProjectDocuments: {
          ...
          onClick: () => this.onViewProjectDocuments(),
        },
        editProject: {
          type: "normal",
          text: "Edit Project",
        ...
        archiveProject: {
          ...
          onClick: () => this.onArchiveProject(),
        },
        editEvent: {
          type: "normal",
          text: "Edit Event",
          onClick: (ev) => this.onEditEvent(ev),
        },
        deleteEvent: {
          type: "normal",
          text: "Delete Event",
    ...
    this.appointmentContextMenuItems = [
      ...
      {
        text: "View project documents",
        onItemClick: that.onViewProjectDocumentsCtx,
      },
  ...
  loadEvents() {
    const timezone = this.dataStore.currentCustomer
    ...
    this.myCalenderApi
      ...
      .then((res: any[]) => {
        const events = res.map((event) => {
          ...
          if (
            event.calendar_event_end_datetime ===
            event.calendar_event_start_datetime
          ) {
        ...
        const {
          currentUser: { user_id, customer_office_id },
        } = this.dataStore;
        if (this.calendarViewMode === "my-active") {
          this.events = events.filter(
            ({ event }) =>
              event["calendar_event_organizer_user_id"] === user_id &&
              event["status"] === "active"
          );
          ...
          this.events = events.filter(
            ({ event }) =>
              event["calendar_event_company_office_id"] ===
                customer_office_id && event["status"] === "active"
          );
  ...
  loadEventOverview(event: any) {
    ...
    if (event.project_id) {
      Promise.all([
        ...
        .then((res: any[]) => {
          ...
          this.overviewData = [
            ...
            {
              name: "Bid Due Date:",
              value: moment(new Date(projectInfo.project_bid_datetime)).format(
                "YYYY-MM-DD HH:mm z"
              ),
            },
            {
              name: "Last Update:",
              value: moment(new Date(projectInfo.edit_datetime_origin)).format(
                "YYYY-MM-DD HH:mm z"
              ),
            },
            {
          ...
          return this.myCalenderApi.findCalendarEvents(
            "",
            event.project_id,
            timezone
          );
        ...
        .then((res: any) => {
          const prebidDate = res.find(
            (event) =>
              event.calendar_event_type === "project_prebid_mtg_datetime"
          );
          const expectedAwardDate = res.find(
            (event) =>
              event.calendar_event_type === "project_expected_award_datetime"
          );
          const projectStartDate = res.find(
            (event) => event.calendar_event_type === "project_start_datetime"
          );
          const workStartDate = res.find(
            (event) =>
              event.calendar_event_type === "project_work_start_datetime"
          );

          if (prebidDate) {
            this.overviewData.push({
              name: "Pre-Bid Meeting:",
              value: this.projectApi
                .convertToTimeZoneObject(
                  prebidDate.calendar_event_start_datetime,
                  timezone
                )
          ...
          if (expectedAwardDate) {
            this.overviewData.push({
              ...
              value: this.projectApi
                .convertToTimeZoneObject(
                  expectedAwardDate.calendar_event_start_datetime,
                  timezone
                )
                .format("YYYY-MM-DD HH:mm:ss z"),
            });
          ...
          if (projectStartDate) {
            this.overviewData.push({
              ...
              value: this.projectApi
                .convertToTimeZoneObject(
                  projectStartDate.calendar_event_start_datetime,
                  timezone
                )
                .format("YYYY-MM-DD HH:mm:ss z"),
            });
          ...
          if (workStartDate) {
            this.overviewData.push({
              ...
              value: this.projectApi
                .convertToTimeZoneObject(
                  workStartDate.calendar_event_start_datetime,
                  timezone
                )
                .format("YYYY-MM-DD HH:mm:ss z"),
            });
        ...
        .then((res: any[]) => {
          this.overviewData.push({
            ...
            value: res
              .map((attendee) =>
                `${attendee.user_firstname} ${attendee.user_lastname}`.trim()
              )
              .join(", "),
          });

          if (event["calendar_event_company_office_id"]) {
            return this.officeApiService.getOffice(
              event["calendar_event_company_office_id"]
            );
          } else {
            return new Promise((resolve) =>
              resolve({ company_office_name: "--" })
            );
          }
        })
        .then((eventOffice) => {
      ...
      this.authApi
        .getUserById(event.calendar_event_organizer_user_id)
        .then((res: any) => {
          this.overviewData = [
            {
              name: "Event Name:",
            ...
            },
            {
              ...
              value: moment(
                new Date(event.calendar_event_start_datetime)
              ).format("YYYY-MM-DD HH:mm:ss z"),
            },
            {
              name: "Event End Date/Time:",
              value: event.calendar_event_end_datetime
                ? moment(new Date(event.calendar_event_end_datetime)).format(
            ...
            {
              name: "Last Update:",
              value: moment(new Date(event.edit_datetime)).format(
                "YYYY-MM-DD HH:mm:ss z"
              ),
            },
          ];
    ...
    if (this.dataStore.currentUser["customer_office_id"]) {
      this.officeApiService
        ...
        .then((office) => {
          ...
          this.calendarViewModes = [
            { value: "my-active", label: "My Events" },
            { value: "my-inactive", label: "My Inactive Events" },
            this.currentOffice && {
              value: "office-active",
              label: `${this.currentOffice.company_office_name} Events`,
            },
            this.currentOffice && {
              value: "office-inactive",
              label: `${this.currentOffice.company_office_name} Inactive Events`,
            },
            { value: "all-active", label: "All Events" },
            { value: "all-inactive", label: "All Inactive Events" },
          ];
  ...
  onCellClick(event) {
    ...
    this.selectedEvent = this.events.find(
      ({ startDate }) =>
        ...
        startDate < event.cellData.endDate
    );
  ...
  onOptionChanged(event: any) {
    if (
      event.name === "currentView" &&
      (event.value === "day" || event.value === "week")
    ) {
      setTimeout(() => {
        this.scheduler.instance.scrollToTime(
          Math.max(this.currentDate.getHours() - 1, 0),
          0
        );
      });
    }
  ...
  onAddEvent(event: any) {
    if (event) {
      ...
      this.notificationService.error("Error", "Please select a cell", {
        timeOut: 3000,
        showProgressBar: false,
      });
    }
  ...
  onEditEvent(event: any) {
    if (event) {
      ...
      this.notificationService.error("Error", "Please select an event", {
        timeOut: 3000,
        showProgressBar: false,
      });
    }
  ...
  onViewProject() {
    if (this.selectedEvent) {
      if (this.selectedEvent.event.project_id) {
        window.open(
          `/customer-portal/view-project/${this.selectedEvent.event.project_id}`,
          "_blank"
        );
      } else {
        this.notificationService.error("Error", "This is not a project event", {
          timeOut: 3000,
          showProgressBar: false,
        });
      }
  ...
  onEditProject() {
    if (this.selectedEvent) {
      if (this.selectedEvent.event.project_id) {
        this.myCalenderApi
          .findProject(this.selectedEvent.event.project_id)
          .then((project) => {
  ...
  onViewProjectDocuments() {
    if (this.selectedEvent) {
      if (this.selectedEvent.event.project_id) {
        ...
        window.open(
          `${window["env"].docViewerBaseUrl}?project_id=${this.selectedEvent.event.project_id}&user_id=${userId}&doc_type=normal&doc_id=unknown&folder_id=unknown`,
          "_blank"
        );
      } else {
        this.notificationService.error("Error", "This is not a project event", {
  ...
  schedulerRefresh() {
    setTimeout(
      () => DomUtils.triggerResizeEvent(this.scheduler.instance.element()),
      0
    );
  }
