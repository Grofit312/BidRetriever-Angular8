...
export class TabsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataStore.authenticationState.subscribe(state => {
      if (state) {
        ...
        this._authApi.findUserDevices(userId)
          .then((devices: any[]) => {
  ...
  onClickTab(event: any, index: number) {
    ...
    switch (index) {
      case 1:
        this._router.navigate([newLoad ? this._router.url : '/customer-portal/my-projects']);
        break;

      case 2:
        this._router.navigate([newLoad ? this._router.url : '/customer-portal/shared-projects']);
        break;

      case 3:
        this._router.navigate([newLoad ? this._router.url : '/customer-portal/my-calendar']);
        break;

      case 4:
        if (this.dataStore.currentCustomer) {
          this._router.navigate([newLoad ? this._router.url : '/customer-portal/my-settings']);
        } else {
      ...
      case 5:
        if (!this.dataStore.currentUser || this.dataStore.currentCustomer) {
          this._router.navigate([newLoad ? this._router.url : '/customer-portal/system-settings']);
        } else {
          event.preventDefault();
          event.stopPropagation();

          document.getElementById('tab-my-projects').click();
          document.getElementById('tab-system-settings').classList.remove('active');
      ...
      case 6:
        this._router.navigate([newLoad ? this._router.url : '/customer-portal/all-submissions']);
        break;
