...
export class EditProjectModalComponent implements OnInit {
      ...
      this.sourceSystemAccountsApi
        ...
        .then((sourceSystemTypes: any) => {
          if (Array.isArray(sourceSystemTypes)) {
            this.sourceSystemTypes = sourceSystemTypes.sort((a, b) =>
              a.source_type_name.toLowerCase() >
              b.source_type_name.toLowerCase()
                ? 1
                : a.source_type_name.toLowerCase() <
                  b.source_type_name.toLowerCase()
                ? -1
      ...
      this.contactApi
        ...
        .then((res: any) => {
          this.contactTypeList = new DataSource({
            store: { data: res, type: "array", key: "contact_id" },
          });
        });
    }
  }
  ...
  onEmailDetail(email) {
    ...
    this.sourceSystemAccountsApi.createContactEmail(params).then((res: any) => {
      ...
      if (res.data) {
        this.notificationService.success("Success", "New contact is created", {
          timeOut: 3000,
          showProgressBar: false,
        });
      }
  ...
  initialize(parent: any, project: any) {
    ...
    this.currentProject["project_auto_update_status"] =
      project["auto_update_status"] === "active";
    this.currentProject["project_bid_datetime"] =
      project["project_bid_datetime"] === "Invalid date"
        ? null
        : project["project_bid_datetime"];
    this.company_name = this.currentProject["source_company_name"];
    this.source_sys_type_id = this.currentProject["source_sys_type_id"];
    ...
    this.userApiService
      ...
      .then((users: any[]) => {
        ...
        return this.calendarApi.findCalendarEvents(
          null,
      ...
      .then((res: any[]) => {
        ...
        this.projectFinishDateTime =
          res.find(
            (event) => event.calendar_event_type === "project_complete_datetime"
          ) || {};
        ...
        this.workFinishDateTime =
          res.find(
            (event) => event.calendar_event_type === "project_work_end_datetime"
          ) || {};
        ...
        this.awardDateTime =
          res.find(
            (event) => event.calendar_event_type === "project_award_datetime"
          ) || {};
        this.expectedContractDateTime =
          res.find(
            (event) =>
              event.calendar_event_type === "project_expected_contract_datetime"
          ) || {};
        this.contractDateTime =
          res.find(
            (event) => event.calendar_event_type === "project_contract_datetime"
          ) || {};

        return this.officeApi.findOffices(
  ...
  onSaveProject() {
    if (
      ...
      !this.currentProject["project_name"].trim()
    ) {
    ...
    const projectOffice = this.offices.find(
      (office) =>
        ...
        this.currentProject["project_assigned_office_id"]
    );
    ...
    if (this.hasNewContact) {
      ...
      const params: any = {
        ...
        company_id: this.companyId
          ? this.companyId
          : this.companyData && this.companyData.company_id
    ...
    const params = Object.assign(
      ...
      { project_name: projectName },
      {
        source_company_id: this.companyId
          ...
          : this.companyData && this.companyData.company_id
          ? this.companyData.company_id
          : this.currentProject.source_company_id,
      },
      {
        source_company_contact_id: this.contactId
          ? this.contactId
          : this.contactEmailDetail
      ...
      { company_website: this.company_website ? this.company_website : "" },
      {
        project_bid_datetime: this.currentProject["project_bid_datetime"]
          ? this.formatDateTime(this.currentProject["project_bid_datetime"])
          : "NULL",
      },
      {
      ...
      {
        auto_update_status: this.currentProject["project_auto_update_status"]
          ? "active"
          : "inactive",
      },
    ...
    this.spinner.show();
    this.apiService
      .updateProject(this.currentProject["project_id"], params)
      .then((res) => this.updateProjectEvents())
      .then((res) => {
        this.spinner.hide();
  ...
  onSourceSystemTypeChange(e) {
    this.source_sys_type_id = e.value;
  }

  updateProjectEvents() {
    const tasks = [];
    tasks.push(
      this.updateProjectEvent(
        this.preBidDateTime,
        "Prebid Meeting Date",
    ...
    if (eventData["calendar_event_start_datetime"]) {
      if (eventData["calendar_event_id"]) {
        return this.calendarApi.updateCalendarEvent(
          ...
          {
            calendar_event_start_datetime: this.formatDateTime(
              eventData["calendar_event_start_datetime"]
            ),
          }
        );
      } else {
        return this.calendarApi.createCalendarEvent({
          ...
          calendar_event_company_office_id:
            this.currentProject["project_assigned_office_id"] || "",
        });
      }
    } else {
